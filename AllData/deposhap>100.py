...
# 修改：筛选Active_Area >= 100的数据
filtered_df = df[df['Active_Area'] >= 100].copy()
print(f"筛选后数据量: {len(filtered_df)}条记录 (Active_Area ≥ 100)")
...
# 打印统计结果
print("\n=== 各类沉积方式数量 (Active_Area ≥ 100) ===")
for _, row in deposition_counts.iterrows():
    print(f"{row['Method_Name']} (编码: {row['Encoded_Method']}): {row['Count']}条记录")

# 保存统计结果到CSV
deposition_counts.to_csv('deposition_methods_count_active_area_ge_100.csv', index=False)
print("\n沉积方式数量统计已保存到: deposition_methods_count_active_area_ge_100.csv")
...
# ============== 主程序 ==============
print("开始分析三种沉积方法的SHAP重要性 (使用集成模型, Active_Area ≥ 100)...")
...
if shap_table_data:
    ...
    output_csv_file = 'deposition_methods_ensemble_shap_active_area_ge_100.csv'
...
if shap_results:
    ...
    for i, (method_name, result) in enumerate(shap_results.items()):
        ...
        # 绘制水平柱状图 - 给每个柱子加上0.8pt的边框
        ax = axes[i]
        bars = ax.barh(range(len(importance_df)), importance_df['Importance'],
                       color=bar_color, alpha=0.8, edgecolor='black', linewidth=0.8)

        # 设置y轴标签（使用别名）- 字体大小设置为10
        ax.set_yticks(range(len(importance_df)))
        ax.set_yticklabels(importance_df['Feature'], fontsize=10)

        # 在所有子图下方显示x轴标签和刻度值 - 字体大小设置为10
        ax.set_xlabel('mean(|SHAP value|)', fontsize=10)

        # 设置坐标轴刻度方向向里 - 字体大小设置为10
        ax.tick_params(axis='x', direction='in', labelsize=10, width=0.8)
        ax.tick_params(axis='y', direction='in', labelsize=10, width=0.8)

        # 确保x轴刻度值在所有子图下方都显示
        ax.tick_params(axis='x', bottom=True, labelbottom=True)

        # 设置子图标题，显示方法名和数据量信息 - 字体大小设置为10
    ...
    if any(ax.has_data() for ax in axes):
        output_plot_file = 'deposition_methods_ensemble_shap_importance_active_area_ge_100_no_depo.tif'
...
# 显示每种方法的最重要特征
print("\n=== 各方法最重要特征 (使用集成模型, Active_Area ≥ 100) ===")
for method_name, result in shap_results.items():
    features = result['features']
    importances = result['mean_abs_shap']
