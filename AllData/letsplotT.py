...
def letsplot(train, trainpre, test, testpre, modelname='RF', target='Bandgap', save_fig=True, save_path=None):
    ...
    a = plt.scatter(train, trainpre, s=25, c="#4d8f74")  # Modify training set color to green
    ...
    b = plt.scatter(test, testpre, s=25, c="#a94837", marker='D')  # Modify test set color to red

    # Remove legend border
    plt.legend((a, b), ('Train', 'Test'), fontsize=fontsize - 1,
               handletextpad=0.1, borderpad=0.1, frameon=False)

    # Set x and y axes with 0.5 increments
    ...
    # Generate ticks with 0.5 step
    ticks = np.arange(min_val, max_val + 0.5, 0.5)
    plt.xticks(ticks)
    plt.yticks(ticks)

    # Ensure x and y axis ranges are consistent
    ...
    if save_fig:
        ...
        # Save as high-resolution TIFF format, 600 DPI
...
def myscatterplot(y_train, y_train_hat, y_test, y_test_hat, modelname="ML", target="PCE", plot_height=8, savepic=False,
    ...
    # Use data directly, no external function calls
    ...
    # Fix: Use correct uppercase PCE, no capitalize() call
    ...
    # Note: R with $R$ for italic, R² with $R^2$
    ...
    # Use MathText rendering to ensure math symbols display correctly
...
# If you need to be fully compatible with the original calling method but use new plotting function, here is an enhanced version
def myscatterplot_enhanced(y_train, y_train_hat, y_test, y_test_hat, modelname="ML", target="PCE", plot_height=8,
    ...
    print(f"  Training set sample count: {len(y_train_flat)}")
    print(f"  Test set sample count: {len(y_test_flat)}")
    ...
    plt.rcParams['font.family'] = 'Times New Roman'
    plt.rcParams['mathtext.fontset'] = 'cm'  # Use Computer Modern font to render math symbols
