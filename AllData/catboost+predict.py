...
def predict_bandgap(element_ratios):
    ...
    try:
        ...
        # 打印模型信息
        print(f"📋 Bandgap模型特征数量: {bandgap_model.feature_count_}")
...
def prepare_sample_data(sample_data, mapping_df, historical_data, use_predicted_bandgap=True, predicted_bandgap=None):
    ...
    if use_predicted_bandgap and predicted_bandgap is not None:
        ...
        print("\n🔬 开始Bandgap预测...")
        element_ratios = {
            'FA': sample_data.get('FA', 0),
            'MA': sample_data.get('MA', 0),
            'Cs': sample_data.get('Cs', 0),
            'I': sample_data.get('I', 0),
            'Br': sample_data.get('Br', 0),
            'Cl': sample_data.get('Cl', 0),
            'Pb': sample_data.get('Pb', 0)
        }
        ...
        if predicted_bandgap is not None:
            ...
            # 如果Bandgap预测失败，使用合理的默认值
            sample_data['Bandgap'] = 1.60  # 钙钛矿典型的bandgap值
            print(f"⚠️  Bandgap预测失败，使用默认值: {sample_data['Bandgap']:.4f} eV")
    else:
        # 使用给定的Bandgap值
    ...
    feature_name_corrections = {
        'HTL-Addictive': 'HTL-Additive',
        'ETL-Addictive': 'ETL-Additive',
        'Precursor_Solution_Addictive': 'Precursor_Solution_Additive'
    }

    for wrong_name, correct_name in feature_name_corrections.items():
        if wrong_name in new_sample.columns:
            # 重命名特征
            new_sample = new_sample.rename(columns={wrong_name: correct_name})
            print(f"✅ 已重命名特征: {wrong_name} -> {correct_name}")
    ...
    categorical_features = [
        'Structure', 'HTL', 'HTL-2', 'HTL_Passivator', 'HTL-Additive',
        'ETL', 'ETL-2', 'ETL_Passivator', 'ETL-Additive',
        'Metal_Electrode', 'Glass', 'Precursor_Solution',
        'Precursor_Solution_Additive', 'Deposition_Method',
...
def predict_pce_for_new_samples():
    ...
    # 2. 加载历史数据以获取特征结构
    try:
        historical_data = pd.read_excel('FinalData10312All.xlsx')
        print("✅ 历史数据加载成功")
    ...
    sample1_data = {
        ...
        'Precursor_Solution_Additive': '',  # 修正拼写
    ...
    sample2_data['HTL-Additive'] = ''  # HTL-Additive改为空值
    sample2_data['Glass'] = 'ITO'

    # 6. 准备第三组数据（HTL-2和HTL-Additive都变为空值）
    sample3_data = sample1_data.copy()  # 复制第一组数据
    sample3_data['HTL-2'] = ''  # HTL-2改为空值
    sample3_data['HTL-Additive'] = ''  # HTL-Additive改为空值
    sample3_data['Glass'] = 'ITO'

    # 7. 准备第四组数据（HTL-Additive换成空值）
    sample4_data = sample1_data.copy()  # 复制第一组数据
    sample4_data['HTL-Additive'] = ''  # HTL-Additive改为空值
    sample4_data['Glass'] = 'ITO'

    # 8. 准备第五组数据（HTL-Additive换成空值，同时ETL_Passivator变成LiF）
    sample5_data = sample1_data.copy()  # 复制第一组数据
    sample5_data['HTL-Additive'] = ''  # HTL-Additive改为空值
    sample5_data['ETL_Passivator'] = 'LiF'  # ETL_Passivator改为LiF
    sample5_data['Glass'] = 'ITO'

    # 9. 准备第六组数据（HTL-Additive变成DMPU+PEAI，同时ETL_Passivator变成LiF）
    sample6_data = sample1_data.copy()  # 复制第一组数据
    sample6_data['HTL-Additive'] = 'DMPU+PEAI'  # HTL-Additive改为DMPU+PEAI
    ...
    print("\n" + "=" * 60)
    print("🎯 第三组数据预测 (HTL-2: 空值, HTL-Additive: 空值)")
    print("=" * 60)
    print(f"使用第一组数据预测的Bandgap: {first_sample_bandgap:.4f} eV")
    ...
    print("\n" + "=" * 60)
    print("🎯 第五组数据预测 (HTL-Additive: 空值, ETL_Passivator: LiF)")
    print("=" * 60)
    print(f"使用第一组数据预测的Bandgap: {first_sample_bandgap:.4f} eV")
    ...
    print("\n" + "=" * 60)
    print("🎯 第六组数据预测 (HTL-Additive: DMPU+PEAI, ETL_Passivator: LiF)")
    print("=" * 60)
    print(f"使用第一组数据预测的Bandgap: {first_sample_bandgap:.4f} eV")
...
if __name__ == "__main__":
    ...
    print("本系统将预测六种不同配置的PCE性能")
    print("配置1 (基准): HTL = NiOx, HTL-2 = Me-4PACz, HTL-Additive = DMPU, ETL_Passivator = 空值")
    print("配置2: HTL = Me-4PACz, HTL-2 = 空值, HTL-Additive = 空值, ETL_Passivator = 空值")
    print("配置3: HTL = NiOx, HTL-2 = 空值, HTL-Additive = 空值, ETL_Passivator = 空值")
    print("配置4: HTL = NiOx, HTL-2 = Me-4PACz, HTL-Additive = 空值, ETL_Passivator = 空值")
    print("配置5: HTL = NiOx, HTL-2 = Me-4PACz, HTL-Additive = 空值, ETL_Passivator = LiF")
    print("配置6: HTL = NiOx, HTL-2 = Me-4PACz, HTL-Additive = DMPU+PEAI, ETL_Passivator = LiF")
    ...
    if results:
        ...
        config_names = {
            'sample1': "HTL = NiOx, HTL-2 = Me-4PACz, HTL-Additive = DMPU, ETL_Passivator = 空值",
            'sample2': "HTL = Me-4PACz, HTL-2 = 空值, HTL-Additive = 空值, ETL_Passivator = 空值",
            'sample3': "HTL = NiOx, HTL-2 = 空值, HTL-Additive = 空值, ETL_Passivator = 空值",
            'sample4': "HTL = NiOx, HTL-2 = Me-4PACz, HTL-Additive = 空值, ETL_Passivator = 空值",
            'sample5': "HTL = NiOx, HTL-2 = Me-4PACz, HTL-Additive = 空值, ETL_Passivator = LiF",
            'sample6': "HTL = NiOx, HTL-2 = Me-4PACz, HTL-Additive = DMPU+PEAI, ETL_Passivator = LiF"
        }
