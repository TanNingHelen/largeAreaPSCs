...
class ScribingOptimizer:
    ...
    def _prepare_input_data(self, params):
        ...
        base_data = {
            ...
            'HTL-2': 'Me-4PACz',
            'HTL_Passivator': 'PEAI',
            'HTL-Addictive': 'DMPU',  # 统一使用Addictive
            'ETL': 'C60',
            'ETL-2': 'SnO2',
            'ETL_Passivator': '',
            'ETL-Addictive': '',  # 统一使用Addictive
            'Metal_Electrode': 'Cu',
            'Glass': 'FTO',
            'Perovskite': '(FA0.98MA0.02)0.95Cs0.05Pb(I0.98Br0.02)3',
            'Active_Area': 12.96,
            'Precursor_Solution': 'DMF:NMP (7:1)',
            'Precursor_Solution_Addictive': '',  # 统一使用Addictive
    ...
    def optimize_parameters(self):
        ...
        if results:
            ...
            self._save_results(top_500_results, results_df)
    ...
    def _generate_line_chart(self, results_df):
        ...
        try:
            ...
            if len(high_pce_data) > 0:
                # 将High PCE threshold线换成深蓝色，线宽与Baseline PCE保持一致（线宽2）
    ...
    def _save_results(self, top_500_results, all_results_df):
        """保存结果"""
        try:
            # 为top 500结果添加改进百分比
            top_500_results['Improvement_Percentage'] = (
                    (top_500_results['Predicted_PCE'] - self.baseline_pce) / self.baseline_pce * 100)
            top_500_results['Improvement_Absolute'] = (top_500_results['Predicted_PCE'] - self.baseline_pce)
            ...
            for col in columns_order:
                if col not in top_500_results.columns:
                    top_500_results[col] = 0

            # 重新排列列的顺序
            top_500_results = top_500_results[columns_order]

            filename = f"{self.results_dir}/top_500_optimized_parameters_{self.timestamp}.csv"
            top_500_results.to_csv(filename, index=False, encoding='utf-8-sig')
            print(f"💾 Top 500 results saved: {filename}")

            # 🔥 新增：导出用于Origin作图的数据文件（包含所有点和高PCE标记）
            origin_data_filename = f"{self.results_dir}/origin_plot_data_{self.timestamp}.csv"

            # 计算高PCE阈值（前500个点的PCE值）
            high_pce_threshold = all_results_df.nlargest(500, 'Predicted_PCE')['Predicted_PCE'].min()

            # 创建包含高PCE标记的完整数据集
            origin_df = all_results_df[['Total_Width', 'Predicted_PCE']].copy()
            origin_df['Is_High_PCE'] = origin_df['Predicted_PCE'] >= high_pce_threshold
            origin_df['High_PCE_Marker'] = origin_df['Is_High_PCE'].map({True: 'High PCE', False: 'Normal'})

            origin_df.to_csv(origin_data_filename, index=False, encoding='utf-8-sig')
            print(f"📊 Origin plot data saved: {origin_data_filename}")
            print(f"   - Total points: {len(origin_df)}")
            print(f"   - High PCE points: {origin_df['Is_High_PCE'].sum()}")
            print(f"   - Normal points: {len(origin_df) - origin_df['Is_High_PCE'].sum()}")
            print(f"   - High PCE threshold: {high_pce_threshold:.4f}%")

            self._generate_report(top_500_results)
