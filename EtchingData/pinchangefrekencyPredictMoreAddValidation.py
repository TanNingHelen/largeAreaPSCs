...
class ScribingOptimizer:
    ...
    def _generate_line_chart(self, results_df):
        ...
        try:
            ...
            validation_x = [point[0] for point in validation_points]
            validation_y = [point[1] for point in validation_points]

            plt.scatter(validation_x, validation_y,
                        marker='^', s=100, color='purple',
                        label='Validation data', edgecolors='black', linewidth=1.5)
            ...
            # 调整y轴上限以显示验证数据点
            y_min = sorted_results['Predicted_PCE'].min() - 0.5
            y_max = max(sorted_results['Predicted_PCE'].max(), max(validation_y)) + 0.5
